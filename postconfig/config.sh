...
## Hyprland
# Config
cp -r ~/sysconfig/hypr ~/.config/
cp -r ~/sysconfig/rofi ~/.config/
...
cp ~/sysconfig/starship/starship.toml ~/.config/
...
# Breeze cursor
unzip ~/sysconfig/cursors/BreezeX-Amber.zip -d ~/.local/share/icons/
unzip ~/sysconfig/cursors/BreezeX-Amber-Hyprcursor.zip -d ~/.local/share/icons/
...
# Copy Yazi config
cp -rp ~/sysconfig/yazi ~/.config/

# Install packages from AUR
paru -S hyprshot zen-browser-avx2-bin sublime-text-4 bashmount